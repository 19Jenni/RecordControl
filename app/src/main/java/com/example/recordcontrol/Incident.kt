...
data class Incident(val employee: Empleoye,
    ...
    fun getAbsences(): Int {
        ...
        val isApplyRetardant = employee.getSeniority()< Constants.MINIMUM_SENIORITY_FREE_RETARDANT
        val retardant =  getRetardant()
        val absencesForRetardant = when(isApplyRetardant){
            true -> (retardant / Constants.NUMBER_OF_RETARDANT_EQUIVALENT_TO_ONE_ABSENCE)
            else ->0
        }
